...
$(document).ready(function () {
    ...
    let postname;
    let postphone;
    let calendarID = '4d29e68cb3c08e2da18abda34b96dee2eef449e63a97ea8e8461506f3432d25d@group.calendar.google.com';
    ...
    function appendTime(id, startDateTime, endDateTime){
        const startvalue = getTimeFormatted(startDateTime);
        const endvalue = getTimeFormatted(endDateTime)
    ...
    function showDate(dateText){
        ...
        .done(function (response) {
            ...
            $('#dateInstructions').text('Choose an apppintment time. Availabilities are subject to change.');
    ...
    function finalizeAppointment(){
        ...
        $('#availableTimeContainer').empty();
        $('#scheduleForm').html(`
            <p class="display-8 mb-2">You're Almost Done!</p>
            <div class="d-flex">
    ...
    //"Your consent screen is being shown, but your app is under reviewed so your users may not see all of your information, and you will not be able to request certain OAuth scopes."
    function postEvent(){
        //remove the entire booking window
        $('#bookingWindow').css('display', 'none');
        //show loading spinner
        $('#bookingPosting').css('display', 'flex');
        const calendarID = '4d29e68cb3c08e2da18abda34b96dee2eef449e63a97ea8e8461506f3432d25d@group.calendar.google.com';
        $.ajax({
        ...
        .done(function () {
            $('#finalConsultationMessage').text('Appointment request received. We will reach out to you by email to confirm your appointment shortly.')
            console.log(`Updated calendar event ${selectedID} to private and patched user data`)
        })
        .fail(function (response) {
            $('#finalConsultationMessage').text('Unable to reserve appointment at this time. Please try again later or try sending us a message below.')
            ...
            console.log('Message from google: Your consent screen is being shown, but your app is under reviewed so your users may not see all of your information, and you will not be able to request certain OAuth scopes.');
            console.log(response.responseJSON);
        })
        .always(function(){
            $('#bookingPosting').css('display', 'none');
    ...
    //start off conditions animation by adding the first item in conditions list,
    //then waiting 1 second and then adding exit class to shown condition
    ...
    $('#messagebutton').bind('keydown click', function(event) {
        ...
        if(noErrors){
            //get all values from #contactform and serialize into query string
            const data = $('#contactForm').serialize();

            //show loading spinner, remove the entire contact us region
            $('#messageSending').css('display',  'flex');
            $('#contactUs').css('display',  'none');
            //post data to Google sheets
            //submissions can be viewed here: https://docs.google.com/spreadsheets/d/19ZfzmT8Dsu7MaeL9e9LWkYpV88E8IGrWdW6yzd2KD58/edit?usp=sharing
            $.ajax({
            ...
            //always remove the contact form after to prevent resubmits
            .always(function(){
                $('#messageSending').css('display',  'none');
            });
        }     
    }); 

    /* start of code for making affiliates map*/
    ...
    const userlocation = new LocationIcon({iconUrl: 'media/curLoc.gif'});
    ...
    /* <--- end of code for making affiliates map ---> */

    /* <--- start of code for creating available dates schedule ---> */
    //the calendar that we are getting info can be viewed here: https://calendar.google.com/calendar/u/1?cid=NGQyOWU2OGNiM2MwOGUyZGExOGFiZGEzNGI5NmRlZTJlZWY0NDllNjNhOTdlYThlODQ2MTUwNmYzNDMyZDI1ZEBncm91cC5jYWxlbmRhci5nb29nbGUuY29t
    ...
    /* <--- end of code for createing available dates schedule ---> */
